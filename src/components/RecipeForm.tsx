...
import { addRecipe, editRecipe } from "@/app/(app)/actions";
import { SLOTS, type Recipe, type Tag } from "@/lib/types";
...
export default function RecipeForm({
  ...
  const [tag, setTag] = useState<Tag>(recipe?.tag ?? "Dinner");
  ...
  const [nutrition, setNutrition] = useState<Record<string, string>>(
    Object.fromEntries(
      NUTRITION_FIELDS.map((f) => [
        f.key,
        recipe ? String((recipe as unknown as Record<string, number>)[f.key] ?? "") : "",
      ])
    )
  );
  ...
  const [video, setVideo] = useState<{ url: string; isVideo: boolean; uploading: boolean } | null>(
    recipe?.prep_video_url
      ? { url: recipe.prep_video_url, isVideo: recipe.prep_video_is_video, uploading: false }
      : null
  );
  ...
  function submit() {
    startTransition(async () => {
      ...
      if (mode === "edit" && recipe) {
        await editRecipe(recipe.id, input);
  ...
  const backLabel = mode === "edit" ? "← Back to recipe" : "← Back to recipes";

  return (
    <div className="max-w-2xl">
      <Link href={backHref} className="text-[13px] text-accent no-underline mb-4 inline-block">
        {backLabel}
      </Link>
      <h1 className="mp-title mb-6">{mode === "edit" ? "Edit Recipe" : "Add a Recipe"}</h1>
