...
export default async function DiaryPage() {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="font-script text-4xl font-bold text-accent mb-1">Food Diary</h1>
      <p className="font-serif text-sm text-[#5A5266] mb-7">
        What you actually ate — logged straight from your Meal Plan, with nutrition totaled per day.
      </p>

      {!entries.length && (
        <div className="bg-[#FFFDFB] rounded-2xl p-10 text-center font-serif text-sm text-[#8A8195]">
        ...
        return (
          <div key={date} className="mb-9">
            <div className="flex items-baseline justify-between mb-3">
              <h2 className="font-script text-2xl font-bold text-[#3A3245]">{formatDate(date)}</h2>
            </div>
            ...
            {dayEntries.map((entry) => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <span className="font-script text-xl font-bold text-[#3A3245]">
                    {entry.recipe_name}
                  </span>
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="flex-grow">
                    {entry.actual_eaten && (
                      <p className="font-serif italic text-[13.5px] text-[#5A5266] mb-3">
                        &ldquo;{entry.actual_eaten}&rdquo;
                      </p>
                    )}
                    <div className="flex gap-4 flex-wrap text-[12.5px] text-[#6E6579]">
